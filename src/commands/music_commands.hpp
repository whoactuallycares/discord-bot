...
inline void command_play(dpp::cluster& _bot, const dpp::message_create_t& _event, const std::vector<std::string_view>& _args)
{
    ...
    else if (_args.size() > 1)
    {
		...
		*/
		// play
		//std::cout << (q.voiceConnection ? "true" : "false");
		//std::cout << (q.voiceConnection->voiceclient ? "true" : "false");
		//std::cout << (q.voiceConnection->voiceclient->is_ready() ? "true" : "false");
...
//#ifdef __linux__ 
			...
			for (uint32_t i = 0; i < 100 && !fd; i++)
			{
				...
				fd = fopen(std::string("songs/" + p[0].id + ".opus").c_str(), "rb");
			std::cout << "try\n";
			};
			if (!fd)
			{
